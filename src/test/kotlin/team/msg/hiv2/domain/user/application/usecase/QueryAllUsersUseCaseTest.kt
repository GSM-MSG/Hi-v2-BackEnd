...
@HiTest
internal class QueryAllUsersUseCaseTest {

    @Mock
    private lateinit var userService: UserService

    private lateinit var queryAllUsersUseCase: QueryAllUsersUseCase
    ...
    @BeforeEach
    fun setUp() {
        queryAllUsersUseCase = QueryAllUsersUseCase(
            userService
        )
    }
    ...
    fun `전체 학생 조회 성공`() {
        ...
        // given
        given(userService.queryAllUsers())
            .willReturn(listOf(userStub1, userStub2))

        // when
        val response = queryAllUsersUseCase.execute()
